...
**Home headline typography**: the zh `homeTitle` (欢迎来到鸟舍。) uses **Glow Sans SC Extended Bold** (`src/fonts/GlowSansSCExtended-Bold-headline-subset.woff2`, SIL OFL — welai/glow-sans; code MIT, font OFL, both kept in `assets/fonts/glow-sans/`), loaded via `next/font/local` (`glowSansHeadline` in `src/lib/fonts.ts`) and applied through `.titleZh` in `page.module.css`, same pattern as the podcast page's `.titleZh`. Picked after evaluating Glow Sans's live design-playground (welai.github.io/glow-sans) directly against its actual GitHub releases: the playground's "对比 Contrast" and other fine-tuning sliders only drive a client-side preview (confirmed by reading their bundled JS — no font-file generation code anywhere in it, and their own "save" button exports a settings JSON, not a font) and are never published as installable files. Real building a fully custom instance would mean running their offline pipeline (otfcc + Node + Python, downloads the full Source Han Sans sources, ~4GB RAM, can take hours) — not worth it for one headline, so **Extended** (a real shipped static width) was used as-is instead of chasing the exact tuned parameters. The zh line break (欢迎来到 / 鸟舍。) is a manual `.slice(0, 4)` / `.slice(4)` split in `page.tsx`, not CSS-driven wrapping — `max-width` + natural wrap wasn't reliable for a specific 4-then-rest break given CJK characters don't map cleanly onto the `ch` unit. English `homeTitle` is untouched and still just wraps naturally via `text-wrap: balance`.
